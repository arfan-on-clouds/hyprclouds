...
def extract_main_color(image_path, quality=20, downsample_factor=0.5):
    """
...
def get_default_waybar_paths():
    ...
    return [
        f"{home}/.config/material-you/colors.css",
    ]
...
def generate_css(color_int, output_paths=None):
    ...
    light = Scheme.light(color_int).props  # Use .props instead of .to_dict()
    dark = Scheme.dark(color_int).props  # Use .props instead of .to_dict()
    ...
    # Generate light theme using --light- variables
    css_content += "  /* Light Theme Colors */\n"
    for color in color_keys:
        light_color = light.get(color, [0, 0, 0])
        css_content += f"  --light-{color}: {rgb_to_hex(light_color)};\n"
    ...
    # Generate dark theme using --dark- variables
    css_content += "\n  /* Dark Theme Colors */\n"
    for color in color_keys:
        ...
        css_content += f"  --dark-{color}: {rgb_to_hex(dark_color)};\n"
